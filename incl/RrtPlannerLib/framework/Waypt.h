...
public:
    ...
    Waypt(const Coord_NE& coord, double lon0_deg = 0.0, int id = -1);
    virtual ~Waypt();
    ...
    void set(const Coord_NE& coord, double lon0_deg = 0.0, int id = -1);
    ...
    void setCoord(const Coord_NE& coord); //[m][m] set northing/easting
    const Coord_NE& coord_const_ref() const; //[m][m] northing/easting

    //presence flags of fields
    FieldFlags getFieldFlags() const;

public:
    Vector_NE vectorTo(const Waypt& otherWaypt) const;
